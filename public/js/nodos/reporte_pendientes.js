$(function () {
	var locales = {
		...
		excel_filename :  'Reporte de Nodos Saturados Pendientes'
	};
	...
	$.fn.dataTableExt.sErrMode = 'throw';
     ...
     .DataTable({
		...
		ajax: {
			method: 'GET',
			url: APP_URL + '/nodos/nodos-pendientes',
			data: function(d) {
			...
			dataSrc: function(json) {
				for(var i = 0, ien = json.length ; i < ien ; i++) {
					json[i].consecutivo = '<a href="' + APP_URL + '/nodos/editar/' + json[i].consecutivo + '">' + json[i].consecutivo + '</a>';
					json[i].comentarios = '<span>' + (json[i].comentarios != null? json[i].comentarios: '-') + '</span>';
				}
				return json;
			},
    	...
    	columnDefs: [
    	    { targets: [0,1,2,3,4,6,7,8,12,13,14,15,16,17,18], searchable: false }
    	],
	});
